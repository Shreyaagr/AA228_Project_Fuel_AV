"""
This Code is partially adapted from: https://github.com/yanxi0830/CS238CrosswalkDriving/blob/main/src/simple/QLearning.py
"""
...
class Qlearning:
    ...
    def state_to_index(self, state):
        index = state[1] + (self.env.max_velocity - self.env.min_velocity + 1) * state[0]
        return index
...
def test(env, model, n_steps, random_policy=False):
    ...
    step_to_goal = 0
    success = False
    state_list = [[env.state[0], env.state[1]]]
...
def main():
    ...
    plot([i for i in range(len(state_list_q))], [np.array(state_list_q)[:,-1], [CONFIG['road_grade'][i] for i in np.array(state_list_q)[:,0]]],
          'time', 'velocity / road grade', 'velocity and road grade profile vs time',
          ['velocity', 'road grade'], 'results/qlearning_profile', ylim=True)
    plot([i for i in range(len(state_list_r))], [np.array(state_list_r)[:,-1], [CONFIG['road_grade'][i] for i in np.array(state_list_r)[:,0]]],
          'time', 'velocity / road grade', 'velocity and road grade profile vs time',
